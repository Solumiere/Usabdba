...
<div class="row">
  <aside class="col-md-3 mb-4">
    <form method="GET" class="card card-body">
      <h6>Фильтры</h6>
      <input type="text" name="search" value="<?= e(request('search')) ?>" class="form-control mb-2" placeholder="Поиск...">
      <select name="genre" class="form-select mb-2">
        <option value="">Все жанры</option>
        @foreach ($genres as $g)
          <option value="<?= e($g) ?>" @selected(request('genre') == $g)><?= e($g) ?></option>
        @endforeach
      </select>
      <div class="d-flex gap-2 mb-2">
        <input type="number" name="min_price" value="<?= e(request('min_price')) ?>" class="form-control" placeholder="от">
        <input type="number" name="max_price" value="<?= e(request('max_price')) ?>" class="form-control" placeholder="до">
      </div>
      ...
      @forelse ($games as $game)
        <div class="col-sm-6 col-lg-4">
          <div class="card h-100"><div class="card-body d-flex flex-column">
            <h5 class="card-title"><?= e($game->title) ?></h5>
            <p class="text-muted small mb-1"><?= e($game->genre) ?></p>
